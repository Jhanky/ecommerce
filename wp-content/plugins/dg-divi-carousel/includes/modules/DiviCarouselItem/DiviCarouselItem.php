...
class DiviCarouselItem extends ET_Builder_Module {
	...
	public function get_advanced_fields_config() {
		...
		$advanced_fields['fonts']  = array(
			...
			'subtitle'   => array(
				...
				'css'      => array(
					'main' => ".dica_divi_carousel %%order_class%%.dica_divi_carouselitem .dica-item-content h5",
					'important' => 'all',
				),
			),
			// Body Text
	...
	public function additional_css_styles($render_slug){
		...
		if ( $this->props['mobile_on_top'] === 'on' ) {
			ET_Builder_Element::set_style($render_slug, array(
				'selector' => '%%order_class%%.dica_divi_carouselitem .dica-image-container, %%order_class%%.dica_divi_carouselitem .dica-item-content',
	...
	public function render( $attrs, $content = null, $render_slug ) {
		...
		// create title markup
		if( '' !== $title && '' !== $url) {
			$title = sprintf( '<h4><a href="%1$s"%3$s>%2$s</a></h4>',
				esc_url( $url ),
				esc_html( $title ),
				( 'on' === $url_new_window ? ' target="_blank"' : '' )
			);
		} else if ('' !== $title && '' === $url) {
			$title = sprintf( '<h4>%1$s</h4>',
				esc_html( $title )
			);
		}
		...
		if( 'on' === $this->props['use_social_media'] ) {
			...
			</div>', $facebook, $twitter, $linkedin, $instagram);
		}
		// rating content
